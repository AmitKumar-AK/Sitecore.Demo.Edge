...
$ready = Invoke-Sqlcmd -ConnectionString $ConnectionString -Query "select name from sys.databases where name = 'platform_init_ready'"
if (-not $ready) {
    ...
    Invoke-Sqlcmd -ConnectionString $ConnectionString -InputFile "C:\sql\DisableSitecoreAdminUser.sql" -Verbose
    ...
    Invoke-Sqlcmd -ConnectionString $ConnectionString -Query "create database platform_init_ready" -Verbose
}
